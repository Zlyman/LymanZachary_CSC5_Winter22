...
int main(int argc, char** argv) {
    ...
    const int ANSWR=100;
    unsigned int random;
    int space,indx,sglNum,gCount,games;     
    ...
    for(unsigned int i=0;i<select.size();i++){
        ...
        bet.clear();
        betNum.clear();
        //Call game count function
        games=games+gameCnt(gCount);
    }while(sel!='q'&&sel!='Q');
     cout<<"-----------------------BET--LOG-----------------------"<<endl;
...
string asgnClr(int array[][38],int size,int indx){
    int colNum;
    string color;
    colNum=array[1][indx];
    switch(colNum){
...
int asgnNum (int array[][38],int size,int indx){
    int space;
    space=array[0][indx];
    return space;
}

int spinWhl(int array[][38],int size,int random){
    float indx;
    indx=random%38;
    return floor(indx);
}